...
@client_staff_leave_bp.route("/summary", methods=["GET"])
@require_client_staff_auth
def leave_summary():
    """
    Own-leave summary for the mobile Leave screen's stat cards --
    annual quota/taken/remaining (paid + unpaid), or a single month's
    taken-only figures.

    Query params:
      period: "year" (default) | "month"
      year:   4-digit calendar year, only used when period=year
              (defaults to the current year)
      month:  "YYYY-MM", only used when period=month
              (defaults to the current month)

    Delegates entirely to get_client_staff_leave_summary, which reuses
    list_client_leave_requests + get_leave_type_allocations -- the same
    two reads the Client Dashboard's Leave History tab makes -- so this
    can never disagree with what the same employee's row already shows
    there. org_id/branch_id/staff_id all come from g.client_staff
    (verified JWT), same isolation guarantee as every other route here.
    """
    def _run():
        ...
        if period == "month":
            ...
            if month_param:
                try:
                    year, month_num = (int(part) for part in month_param.split("-", 1))
        ...
        summary = support_cp_db.get_client_staff_leave_summary(
            org_id=g.client_staff["org_id"],
            staff_id=g.client_staff["id"],
            branch_id=g.client_staff.get("branch_id"),
            period=period,
