...
const AdminOnlyRoute: React.FC<{
  children: React.ReactNode;
  // Where to send a staff account instead, appended after their own
  // branch id — e.g. "/settings" so /admin/settings redirects a staff user
  // straight to their own branch's Settings screen rather than dropping
  // them on the branch overview and making them navigate again.
  staffRedirectPath?: string;
}> = ({ children, staffRedirectPath = "" }) => {
  const { user } = useAuth() as { user?: AuthUser | null };
  ...
  if (!isAdminUser(user)) {
    ...
    return (
      <Navigate
        to={branchId ? `/admin/branch/${branchId}${staffRedirectPath}` : "/login"}
        replace
      />
    );
  }
...
export const router = createBrowserRouter([
  ...
  },
  {
    ...
    children: [
      ...
      {
        // Global Settings ("all branches" view) is admin-only, same gate as
        // /admin/branches above: a staff account — even one holding the
        // "settings" module grant — is always scoped to their own branch,
        // never the whole org. Without this wrapper, a staff user could
        // reach the org-wide view by navigating to /admin/settings directly,
        // bypassing the branch isolation enforced everywhere else via
        // BranchAccessRoute/ModuleAccessRoute.
        path: "settings",
        element: (
          <AdminOnlyRoute staffRedirectPath="/settings">
            {suspense(<AdminSettings />)}
          </AdminOnlyRoute>
        ),
      },
      {
