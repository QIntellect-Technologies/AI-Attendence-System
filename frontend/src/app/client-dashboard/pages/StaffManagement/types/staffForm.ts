...
export interface StaffFormData {
  ...
  /**
   * Real backend department UUID (departments.id), selected from the live
   * "Assigned Department" dropdown — workforce-only (see
   * peopleModel.showDepartmentDesignationFields), and the sole source of
   * truth for that dropdown's value in both Add and Edit. Empty string
   * means "unassigned". For an existing staff member this is applied
   * immediately (see handleLiveDepartmentChange); for a new one it's
   * applied right after creation (see handleSave) — same pattern as
   * liveShiftId above. The legacy `department` string field above is no
   * longer written to by this form; it survives only for whatever else
   * still reads it until that's migrated too.
   */
