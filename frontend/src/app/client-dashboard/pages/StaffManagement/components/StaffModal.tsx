...
//   type DepartmentDesignationRecord,
//   type ShiftRecord,
...
//         });
//       loadLiveDesignations(
//         String(
//           (initial as any)?.departmentId ??
//             (initial as any)?.department_id ??
//             "",
//         ),
//       );
//     } else {
...
//         const rows = await listDepartmentDesignations(
//           departmentId,
//           organizationId,
//         );
//         setLiveDesignations(rows);
//       }
//     } catch (error) {
...
//   const loadLiveDesignations = async (departmentId: string) => {
//     if (!departmentId || !organizationId) {
//       setLiveDesignations([]);
//       return;
//     }
//     try {
//       setLiveDesignations(
//         await listDepartmentDesignations(departmentId, organizationId),
//       );
//     } catch {
//       setLiveDesignations([]);
//     }
...
//                           label: item.designation?.name ?? "Designation",
//                         })),
//                     ]}
...
export const StaffModal: FC<{
  ...
  canGrantAdmin = false,
    ...
    const showGroupField = peopleModel.isStudent && hasField("class", "department");
    ...
    // Profile image is optional for every people type (staff and students
    // alike) — upload is offered but never blocks save.
    const hasSelectedRequiredMedia = (): boolean => true;

    // Classes are the student-side equivalent of the workforce
    // liveDepartments below — configured live (ClassSectionEditor /
    // listBranchClasses), not the legacy cfg.departments array.
    const apiBranchId = useMemo(
      () => resolveApiBranchId(organizationId, form.branchId, cfg.branches),
      [organizationId, form.branchId, cfg.branches],
    );
    ...
    useEffect(() => {
      if (!peopleModel.isStudent || !organizationId || !apiBranchId) {
        setLiveClasses([]);
        return;
      }
      let cancelled = false;
      listBranchClasses(apiBranchId, organizationId)
        .then((rows) => {
    ...
    const selectedLiveClass = useMemo(
      () => liveClasses.find((item) => item.name === form.department) ?? null,
      [liveClasses, form.department],
    );

    const [liveSections, setLiveSections] = useState<SectionRecord[]>([]);

    useEffect(() => {
      if (!peopleModel.isStudent || !organizationId || !selectedLiveClass) {
        setLiveSections([]);
        return;
      }
      let cancelled = false;
      listSections(selectedLiveClass.id, organizationId)
        .then((rows) => {
          if (!cancelled) setLiveSections(rows);
        })
        .catch(() => {
          if (!cancelled) setLiveSections([]);
        });
      return () => {
        cancelled = true;
      };
    ...
    // the legacy `depts`/`roles`+cfg.departments/cfg.roles dropdowns above are
    // student-only (Class/Section). Both the shift list and the department
    // list are fetched — and selectable — for BOTH create and edit, since
    // neither requires an existing staff_id to choose from: for a new record
    // the choice is held on form.liveShiftId / form.liveDepartmentId and
    // applied right after createStaff() resolves (see handleSave in the
    // parent). For an existing record it's still PATCHed immediately on
    // change, same as before (see handleShiftAssignmentChange /
    // handleLiveDepartmentChange). form.liveDepartmentId is the single source
    // of truth for the "Assigned Department" dropdown's value in both modes —
    // there is no separate liveAssignedDepartmentId state to drift out of sync
    // with it.
    //
    // Both fetches resolve the real backend branch UUID first — form.branchId
    // is the local UI ordinal id, and every branch-scoped attendance-settings
    ...
    useEffect(() => {
      ...
      // instead (see `depts`/renderConfigItemFamily above). Skip the fetch
      // entirely for student records rather than fetching and then hiding it.
      // No longer gated on `initial` — Add needs the same list to populate
      // form.liveDepartmentId before a staff_id even exists.
      if (peopleModel.showDepartmentDesignationFields) {
        listBranchDepartments(apiBranchId, organizationId)
          .then((rows) => {
            if (!cancelled) setLiveDepartments(rows);
          })
        ...
        // One-time preload for Edit mode, using the same stable initial-derived
        // id form.liveDepartmentId was seeded from (not the reactive form
        // field itself, so this doesn't refire designation loads on every
        // keystroke elsewhere in the form). In Add mode `initial` is undefined
        // so this resolves to "" and loadLiveDesignations no-ops, exactly
        // right since nothing's selected yet.
        loadLiveDesignations(
          String(
            (initial as any)?.departmentId ??
            (initial as any)?.department_id ??
    ...
    // department right after a new staff member is created). For an existing
    // staff member it additionally PATCHes the assignment immediately, same
    // as handleShiftAssignmentChange above. Either way, a department change
    // always clears the current designation selection and reloads the
    // designation list for the newly-chosen department, since a designation
    // only makes sense scoped to one specific department.
    const handleLiveDepartmentChange = async (departmentId: string) => {
      set("liveDepartmentId", departmentId);
      set("designationId", "");
      setLiveDesignations([]);
      if (departmentId && organizationId) {
        loadLiveDesignations(departmentId);
      }
      ...
      try {
        ...
        // Revert the dropdown to the last known-good value since the PATCH
        // failed — form.liveDepartmentId must not silently claim a department
        // that was never actually persisted for an existing staff member.
        const revertId = String(
          (initial as any)?.departmentId ?? (initial as any)?.department_id ?? "",
        );
        set("liveDepartmentId", revertId);
        loadLiveDesignations(revertId);
      } finally {
        setIsSavingLiveDepartment(false);
      }
    };
    ...
    const loadLiveDesignations = async (departmentId: string) => {
      ...
      try {
        setLiveDesignations(
          await listDesignations(departmentId, organizationId),
        );
      } catch {
        setLiveDesignations([]);
      }
        ...
        >
          ...
          >
            ...
            {/* Class/Section (students only) — showGroupField/showSubGroupField
              ...
              department/designation is rendered in the live section
            ...
            {/*
            Live, backend-synced department + designation assignment — the
            only workforce department/designation UI now (see showGroupField
            above). Shown whenever the branch has real departments
            configured, in both Add and Edit: for an existing staff member
            a change PATCHes immediately (handleLiveDepartmentChange); for a
            new one the choice is held on form.liveDepartmentId and applied
                        ...
                        style={{
                          ...
                          background:
                            liveDashboardScope === "team"
                              ? `${T.amber}12`
                              : T.slate50,
                          border: `1px solid ${liveDashboardScope === "team"
                            ? `${T.amber}40`
                            : T.border
                            }`,
                        }}
                          ...
                          {liveDashboardScope === "team" ? (
                            <>
                              ...
                              <strong>
                                {directReports.length === 0
                                  ? "nobody — an empty list until reports are assigned to them"
                                  : `${directReports.length} direct ${directReports.length === 1
                                    ? "report"
                                    : "reports"
                                  } (plus anyone further below them in the chain)`}
                              </strong>{" "}
                        ...
                        style={{
                          ...
                          background:
                            form.accountRole === "admin"
                              ? `${T.amber}12`
                              : T.slate50,
                          border: `1px solid ${form.accountRole === "admin"
                            ? `${T.amber}40`
                            : T.border
                            }`,
                        }}
