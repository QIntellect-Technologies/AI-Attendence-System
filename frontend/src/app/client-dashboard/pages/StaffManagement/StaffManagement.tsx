...
const StaffDirectory: FC = () => {
  ...
  // Live departments/designations for the filter dropdowns below
  // (departmentOptions/roleOptions). Workforce-only, same gate StaffModal
  // uses (peopleModel.isStudent) — students are filtered via Classes &
  // Sections (cfg.departments), untouched. Branch-scoped to whichever
  // branches are currently in view, using the real backend branch id
  // (getBackendBranchId), since listBranchDepartments is a live-relation
  // endpoint and does not understand the legacy numeric UI branch id.
  ...
  }, [branchFilter, isGlobalDashboard, visibleBranches]);
  ...
  useEffect(() => {
    ...
    Promise.all(
      departmentApiBranchIds.map((branchId) =>
        listBranchDepartments(branchId, organizationId).catch(() => []),
      ),
    ).then((results) => {
      if (!cancelled) setLiveDepartments(results.flat());
    });
    return () => {
      cancelled = true;
    };
  }, [departmentApiBranchIds, organizationId, peopleModel.isStudent]);

  // Designations are owned by exactly one department (see listDesignations),
  // so scope the fetch to whichever department is selected in the filter —
  // or every department currently in view, when none is.
  const [liveDesignationNames, setLiveDesignationNames] = useState<string[]>(
    [],
  );
  ...
  }, [deptFilter, liveDepartments, organizationId, peopleModel.isStudent]);

  // Classes are the student-side equivalent of liveDepartments above —
  // configured live (ClassSectionEditor / listBranchClasses), not in the
  // legacy cfg.departments array.
  ...
  useEffect(() => {
    ...
    Promise.all(
      departmentApiBranchIds.map((branchId) =>
        listBranchClasses(branchId, organizationId).catch(() => []),
      ),
    ).then((results) => {
      if (!cancelled) setLiveClasses(results.flat());
    });
    return () => {
      cancelled = true;
    };
  ...
  // Sections are owned by exactly one class (see listSections), so scope
  // the fetch to whichever class is selected in the filter — or every
  // class currently in view, when none is. Student-side equivalent of
  // liveDesignationNames above.
  const [liveSectionNames, setLiveSectionNames] = useState<string[]>([]);

  useEffect(() => {
    if (!peopleModel.isStudent || !organizationId || liveClasses.length === 0) {
      setLiveSectionNames([]);
      return;
    }
    let cancelled = false;
    const targetClasses =
      deptFilter === "all"
        ? liveClasses
        : liveClasses.filter((cls) => cls.name === deptFilter);
    Promise.all(
      targetClasses.map((cls) =>
        listSections(cls.id, organizationId).catch(() => []),
      ),
    ).then((results) => {
      if (cancelled) return;
      setLiveSectionNames(
  ...
  }, [deptFilter, liveClasses, organizationId, peopleModel.isStudent]);
  ...
  const shiftDefinitions = useMemo(
    () =>
      cfg.staffShiftDefinitions.filter(
        (shift) => shift.id !== "custom" && shift.label !== "Custom",
      ),
    [cfg.staffShiftDefinitions],
  );
  ...
  const departmentOptions = useMemo(() => {
    ...
    const configuredGroupNames = peopleModel.isStudent
      ? liveClasses.map((cls) => cls.name).filter(Boolean)
      : liveDepartments.map((dept) => dept.name).filter(Boolean);
  ...
  }, [branchScopedItems, liveClasses, liveDepartments, peopleModel]);

  const roleOptions = useMemo(() => {
    const roleCounts = new Map<string, number>();

    branchScopedItems.forEach((member) => {
      const role = peopleModel.isStudent
        ? member.role || "Unassigned"
        : member.designationName || "Unassigned";
      roleCounts.set(role, (roleCounts.get(role) ?? 0) + 1);
    });

    const configuredRoleNames = peopleModel.isStudent
      ? liveSectionNames
      : liveDesignationNames;
  ...
  }, [branchScopedItems, liveDesignationNames, liveSectionNames, peopleModel]);
    ...
    async (data: StaffFormData, files?: StaffMediaFiles) => {
      ...
      try {
        ...
        if (editMember && editMember !== "new") {
          ...
          // Same reasoning as the shift assignment just above — the
          // Add modal's "Assigned Department" dropdown can't PATCH a
          // staff_id that doesn't exist yet, so the choice was only staged
          // on data.liveDepartmentId. Apply it now, reusing the exact same
          // assignDepartment() endpoint the edit-mode dropdown already
          // calls (support_db_attendance_settings.assign_staff_department),
          // so there is exactly one code path that ever assigns a
          // department. Non-fatal, same as shift above. Note: the chosen
          // designation (data.designationId) needs no separate apply step —
          // buildStaffApiPayload already sends it as designation_id on the
          // initial createStaff() call, since client_staff.designation_id
          // is a plain column set at creation, unlike the department
          // relationship which goes through its own assignment endpoint.
          if (data.liveDepartmentId) {
            try {
              await assignDepartment(result.user.id, data.liveDepartmentId);
