...
type OperationalConfig = {
  ...
  roles: Record<string, DesignationItem[]>;
  cameras: Record<string, CameraItem[]>;
  // Branch-keyed, like departments/roles/cameras above — each branch's NVR
  // lives on its own network, so Public IP / NVR-DVR IP / RTSP credentials
  // must not leak between branches. See _branch_network_from_config and
  // _sync_local_node_camera_config in support_db_client_users.py, which
  // already expect (and prefer) this shape; a flat object there is only a
  // legacy fallback applied identically to every branch.
...
/** Single source of truth for "no network configured yet" — reused by
 * emptyConfig below and by networkByBranch's per-branch fallback, so the
 * default rtspPort (and every other field default) can't drift between the
 * two call sites. */
function emptyNetworkConfig(): NetworkConfig {
  return {
    publicIp: "",
    nvrDvrIp: "",
    rtspUsername: "",
...
function emptyConfig(branches: Branch[]): OperationalConfig {
  ...
  branches.forEach((branch) => {
    ...
    roles[String(branch.id)] = [];
    cameras[String(branch.id)] = [];
    network[String(branch.id)] = emptyNetworkConfig();
  });
...
/** Branch-keyed network config, mirroring normalizeBranchRecord above.
 * Accepts the branch-keyed shape support_db_client_users.py's
 * _branch_network_from_config prefers (`{"<branchId>": {...}}`, optionally
 * nested under byBranch/branches/branchConfigs/networkByBranch), and also
 * reads a legacy flat object (one network config for the whole org) as a
 * one-time migration path — applied to every branch on load only, exactly
 * once, before the very next save persists real per-branch values and the
 * flat shape stops being written altogether. */
function networkByBranch(
  rawNetwork: unknown,
  branches: Branch[],
  keyMap: Record<string, string>,
  ...
  branches.forEach((branch) => {
    result[String(branch.id)] = emptyNetworkConfig();
  });

  const networkRecord = asRecord(rawNetwork);
  const nestedContainer = ["byBranch", "branches", "branchConfigs", "networkByBranch"]
    .map((key) => asRecord(networkRecord[key]))
    .find((candidate) => Object.keys(candidate).length > 0);
  const branchKeyed = nestedContainer || networkRecord;

  const isBranchKeyedShape = Object.keys(branchKeyed).some(
    (rawBranchId) =>
      Object.prototype.hasOwnProperty.call(keyMap, rawBranchId) &&
      asRecord(branchKeyed[rawBranchId]) === branchKeyed[rawBranchId],
  );

  if (isBranchKeyedShape) {
    Object.entries(branchKeyed).forEach(([rawBranchId, value]) => {
      const branchId = keyMap[String(rawBranchId)] || String(rawBranchId);
      if (!Object.prototype.hasOwnProperty.call(result, branchId)) return;
      result[branchId] = normalizeNetworkConfig(value);
    });
    return result;
  }

  // Legacy flat shape (or nothing saved yet): apply the same values to
  // every branch so existing data isn't lost, until the next save splits
  // it out per branch.
  const flat = normalizeNetworkConfig(networkRecord);
  branches.forEach((branch) => {
    result[String(branch.id)] = { ...flat };
  });
  return result;
}
...
function CameraSettingsEditor({
  ...
  >;
  network: Record<string, NetworkConfig>;
  setNetwork: React.Dispatch<React.SetStateAction<Record<string, NetworkConfig>>>;
  terminology: Terminology;
  organization?: BootstrapOrganization;
  ...
  useEffect(() => {
    if (defaultBranchId) {
      if (activeBranchId !== defaultBranchId) setActiveBranchId(defaultBranchId);
      return;
    }
    if (branches.length && !branches.some((b) => b.id === activeBranchId)) {
...
export default function Settings() {
  ...
  const [error, setError] = useState<string | null>(null);
  const [savedMessage, setSavedMessage] = useState<string | null>(null);
  const routeParams = useParams<{ branchId?: string }>();
  const routeBranchId = routeParams.branchId ? String(routeParams.branchId) : null;
  const isBranchDashboard = Boolean(routeBranchId);
  const [selectedBranchId, setSelectedBranchId] = useState<string>("");

  // routeBranchId is the ordinal UI branch id from the URL, but every
  // branch-keyed structure downstream (config.departments/roles/cameras,
  // network, and the department/designation API calls) is keyed by the
  // real Supabase branch UUID. branchKeyMap() already builds that ordinal
  // -> UUID mapping for configFromBootstrap — reuse it here instead of
  // comparing the raw ordinal against a UUID.
  const routeBranchBackendId = useMemo(() => {
    if (!routeBranchId) return null;
    const keyMap = branchKeyMap(bootstrap ?? ({} as BootstrapResponse), branches);
    return keyMap[routeBranchId] ?? routeBranchId;
  }, [routeBranchId, bootstrap, branches]);

  const visibleBranches = useMemo(() => {
    if (!isBranchDashboard || !routeBranchBackendId) return branches;
    return branches.filter((b) => String(b.id) === String(routeBranchBackendId));
  }, [branches, isBranchDashboard, routeBranchBackendId]);
        ...
        <div style={{ display: "grid", gap: 18 }}>
          {organizationId ? (
            <DepartmentDesignationEditor
              organizationId={organizationId}
              branches={branches}
              defaultBranchId={routeBranchBackendId ?? undefined}
              hideTabs={isBranchDashboard}
            />
          ) : null}
          ...
          {plan.hasStudentSetup && organizationId ? (
            <ClassSectionEditor
              organizationId={organizationId}
              branches={branches}
              defaultBranchId={routeBranchBackendId ?? undefined}
