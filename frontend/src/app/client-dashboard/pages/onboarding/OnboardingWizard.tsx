...
import { useAuth } from "../../contexts/useAuth";
import { fetchClientJson, loadClientBootstrap } from "../../services/clintApi";
// Same component Settings.tsx uses for its (non-legacy) Departments &
// Designations card — see that file's docstring for why the old free-text
// WorkforceStructureEditor was retired there in favor of this one, which
// writes real department/designation rows via client_attendance_settings_routes.py
// instead of the onboarding-config blob. Reusing it here (instead of the old
// WorkforceStructureStep below, kept but no longer called) means a
// department or designation added during onboarding shows up in the branch's
// own dashboard exactly the same way one added later in Settings does —
// there is only one department/designation data model now, not two.
import DepartmentDesignationEditor from "../Settings/DepartmentDesignationEditor";
import {
  MAX_PHOTO_BYTES,
  MAX_UPLOAD_BYTES,
  checkFileSize,
...
function StudentClassesStep({
  ...
  return (
    <div>
      <h1 style={h1Style()}>Configure {plan.studentGroupPlural.toLowerCase()}</h1>
      <p style={subStyle()}>
        Add the academic structure used for attendance-enabled students. This
        is the same {plan.studentGroupPlural.toLowerCase()} manager you'll see
...
// Replaces WorkforceStructureStep above (kept but no longer called from
// the wizard — see its call site in content() below) with the real
// department/designation manager Settings.tsx uses. Shift scheduling stays
// local wizard state (config.shiftEnabledPeopleTypes) exactly as before —
// only the department/designation half of the old step is replaced.
function WorkforceDepartmentsStep({
  organizationId,
  branches,
  plan,
  shiftEnabledPeopleTypes,
  ...
  const rows: [string, string][] = [
    ...
    ...(plan.showStudentStructure
      ? ([
        [
          plan.studentGroupPlural,
          `Configured per branch — see ${plan.studentGroupPlural}`,
        ],
      ] as [string, string][])
      : []),
    ...(plan.showWorkforceStructure
      ? ([
        [
          `${plan.workforceGroupPlural} & ${plan.workforceRolePlural}`,
          // Saved directly per branch as you add them on the previous
          // step (same Departments & Designations manager as Settings),
          // not staged in this review's local config -- so there's no
          // local count to show here, same as the branches row above.
          `Configured per branch — see ${plan.workforceGroupPlural}`,
        ],
      ] as [string, string][])
      : []),
    [
...
export default function OnboardingWizard() {
  ...
  const canNext = useMemo(() => {
    ...
    // No local completeness gate here (unlike before): StudentClassesStep's
    // ClassSectionEditor saves each class/section straight to the backend
    // as it's added, the same as WorkforceDepartmentsStep, rather than
    // staging it in config.departments for this step to count.

    // No local completeness gate here (unlike student_structure above):
    // WorkforceDepartmentsStep's DepartmentDesignationEditor saves each
    // department/designation straight to the backend as it's added, the
    // same as Settings.tsx, rather than staging it in config.departments/
    // config.roles for this step to count. Settings has no such gate
    // either -- departments are optional to configure right now and can
