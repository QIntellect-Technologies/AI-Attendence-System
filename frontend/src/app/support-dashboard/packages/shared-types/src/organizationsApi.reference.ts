...
export const branchesApi = {
  ...
  update: (branchId: string, payload: UpdateBranchPayload): Promise<Branch> =>
    supportApiClient
      .patch<BranchEnvelope>(
        `/v1/support/branches/${encodeId(branchId)}`,
        payload,
      )
      .then((r) => r.data.branch),
  ...
  downloadInstaller: async (
    ...
    link.href = url;
    link.download = `qintellect-node-installer-${branchId}.zip`;
