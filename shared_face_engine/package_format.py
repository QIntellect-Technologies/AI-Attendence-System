"""
shared_face_engine/package_format.py

This is local_node/package_import.py, relocated here unchanged. It started
as a Local-Node-only concern, but the cloud-mode "import embeddings" button
on the backend now needs to parse the exact same trainer-produced zip
(manifest + checksummed per-person embedding files) directly into Supabase,
with no local node in the loop at all. Duplicating this parser into app.py
would let its checksum/validation logic drift from what Local Node
enforces — exactly the failure mode shared_face_engine already exists to
prevent for detection/quality/matching. One parser, three consumers:
Trainer Desktop (builds), Local Node (imports to SQLite + pushes fallback
copy), Backend (imports straight to Supabase for cloud-mode orgs).

No behavior changed from local_node/package_import.py — only the location.
local_node/package_import.py should now just be:
    from shared_face_engine.package_format import parse_embedding_package
