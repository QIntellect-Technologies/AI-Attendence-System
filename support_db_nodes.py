...
def get_node_health(org_id: str) -> list[dict]:
    ...
    try:
        ...
        keys_result = (
            sb.table('node_api_keys')
            .select('id, branch_id, node_id, last_seen_at, status, created_at')
            .eq('org_id', org_id)
            .eq('status', 'active')
            .execute()
        )
    ...
    for k in (keys_result.data or []):
        keys_by_branch.setdefault(k['branch_id'], []).append(k)

    health = []
    for branch in branches:
        for key in keys_by_branch.get(branch['id']) or [None]:
            payload = {}
            last_seen = None
            ...
            health.append({
                'id': key.get('id') if key else branch['id'],
                'branch_id': branch['id'],
...
# Per-camera live status values shown across the Client Dashboard. Kept as
# module-level constants (not free-floating string literals) so every
# producer/consumer of this value agrees on the exact spelling.
CAMERA_LIVE_STATUS_NORMAL = 'Normal'
CAMERA_LIVE_STATUS_OFFLINE = 'Offline'
CAMERA_LIVE_STATUS_NOT_SYNCED = 'Not Synced'


def get_camera_live_status(org_id: str) -> dict[str, dict]:
    """Single source of truth for per-camera live health.

    Derived entirely from each branch's local node heartbeat — this is the
    only place that reads node_api_keys.last_heartbeat_payload['cameras']
    (written by node_heartbeat(), populated by camera_stream_manager.
    list_cameras() on the node — see that function's docstring). Nothing
    else should compute camera status independently; both
    get_client_bootstrap() and /api/cctv/live-tracking call
    apply_camera_live_status() below so they can never disagree about the
    same camera.

    Returns {camera_id: {'status': ..., 'lastSeen': iso str | None}}, keyed
    by branch_cameras.id — the same id space the local node itself emits
    (get_node_config() -> _local_node_camera_view() reads branch_cameras
    directly), so callers whose camera list also comes from branch_cameras
    (list_client_cameras()) can merge this by id with no translation step.

    Status rules, using the exact node-offline threshold/computation
    get_node_health() uses (_compute_node_status) so a node the Node Health
    page shows offline can never show its cameras as Normal here:
      - Node has never sent a heartbeat, or a specific camera has never
        appeared in one (e.g. just added, node hasn't polled its updated
        config yet): 'Not Synced'.
      - Node's heartbeat is stale past the org's offline threshold: every
        camera last reported by that node is 'Offline' (see
        camera_stream_manager.list_cameras() docstring — "absence is what
        the dashboard treats as offline").
      - Node heartbeat is fresh: each camera is 'Normal' or 'Offline'
        exactly per that camera's own 'online' flag in the snapshot (which
        itself only means a frame was actually decoded recently — not just
        that the reader thread exists and is retrying).
    """
    from support_db_organizations import get_organization
    sb = get_supabase()
    org = get_organization(org_id)
    ...
    keys_result = (
        sb.table('node_api_keys')
        .select('branch_id, last_seen_at, last_heartbeat_payload')
        .eq('org_id', org_id)
        .eq('status', 'active')
        .execute()
    )

    status_by_camera_id: dict[str, dict] = {}
    for key in (keys_result.data or []):
        node_status, _minutes = _compute_node_status(key.get('last_seen_at'), threshold_seconds)

        payload = key.get('last_heartbeat_payload') or {}
        if not isinstance(payload, dict):
            payload = {}
        cameras_snapshot = payload.get('cameras')
        cameras_snapshot = cameras_snapshot if isinstance(cameras_snapshot, list) else []

        for cam in cameras_snapshot:
            if not isinstance(cam, dict) or not cam.get('id'):
                continue
            camera_id = str(cam['id'])
            last_seen = cam.get('last_frame_at')
            if node_status != 'online':
                # The node itself is down (or never connected) — every
                # camera it last reported is presumed offline with it,
                # regardless of what 'online' said in that last snapshot.
                status = CAMERA_LIVE_STATUS_OFFLINE
            else:
                status = (
                    CAMERA_LIVE_STATUS_NORMAL if cam.get('online')
                    else CAMERA_LIVE_STATUS_OFFLINE
                )
            status_by_camera_id[camera_id] = {'status': status, 'lastSeen': last_seen}

    return status_by_camera_id


def apply_camera_live_status(cameras: list[dict], org_id: str) -> list[dict]:
    """Merge get_camera_live_status() onto any camera list sourced from
    branch_cameras (list_client_cameras() / get_node_config()) — the one
    place both /api/client/bootstrap and /api/cctv/live-tracking attach
    live status, so a camera's status can never be computed two different
    ways in two different places.

    A camera absent from the live-status map (new, or its node has never
    sent a heartbeat) is explicitly 'Not Synced' rather than left without a
    status — never silently defaulted to 'Normal'.
    """
    live_status = get_camera_live_status(org_id)
    result = []
    for camera in cameras:
        camera_id = str(camera.get('id') or camera.get('camera_id') or '')
        live = live_status.get(camera_id)
        status = live['status'] if live else CAMERA_LIVE_STATUS_NOT_SYNCED
        last_seen = live['lastSeen'] if live else None
        result.append({
            **camera,
            'status': status,
            'lastSeen': last_seen,
...
def activate_node_with_install_token(
    ...
    # Multiple concurrently-active nodes per branch are now supported (e.g.
    # separate machines each covering their own subset of cameras — see
    # branch_cameras.assigned_node_id / camera_context_assignments). We no
    # longer revoke every other active key for the branch on activation.
    #
    # We DO still revoke any existing active key sharing this exact
    # node_label: re-running the installer for the same physical machine
    # (same human-chosen label) is a replace, not a new peer, so the old
    # key for that machine is retired rather than left active and orphaned.
    # A blank/omitted node_label activates as a new node and revokes
    # nothing — the previous "one node per branch" behavior is preserved
    # for branches that only ever install a single node.
    if node_label_clean:
        try:
            (
                sb.table('node_api_keys')
                ...
                .eq('status', 'active')
                .eq('node_label', node_label_clean)
                .execute()
            )
        except Exception as exc:
            logger.warning(f'Could not revoke previous node key with label {node_label_clean!r} for branch {branch_id}: {exc}')
...
def get_local_node_status(org_id: str, branch_id: str | None = None) -> dict | None:
    """
    Get the local node status for an organization, optionally narrowed to
    one branch. offline=True only if EVERY active node in scope is
    offline (a branch can now have more than one active node).
    """
    try:
        sb = get_supabase()
        ...
        if branch_id:
            node_query = node_query.eq('branch_id', str(branch_id))
        node_result = _execute_supabase('get_local_node_status.node', lambda: node_query)
        ...
        try:
            statuses = [
                (node, *_compute_node_status(node.get('last_seen_at'), threshold_seconds))
                for node in nodes
            ]
        except Exception as e:
            logger.warning(f'Could not parse heartbeat time: {e}')
            latest = max(nodes, key=lambda n: n.get('last_seen_at') or '')
            return {'offline': True, 'last_heartbeat': latest.get('last_seen_at'), 'node_id': latest.get('node_id')}

        all_offline = all(status != 'online' for _node, status, _mins in statuses)
        latest_node, _status, _mins = max(statuses, key=lambda t: t[0].get('last_seen_at') or '')
        return {
            'offline': all_offline,
            'last_heartbeat': latest_node.get('last_seen_at'),
            'node_id': latest_node.get('node_id'),
...
def _branch_is_single_node_unassigned(org_id: str, branch_id: str, requesting_node_id: str) -> bool:
    """True only for branches that have never used explicit camera
    assignment: exactly one active node key exists for the branch (this
    requester) AND no camera on the branch has ever been assigned to any
    node. Used solely by get_node_config's backward-compatibility fallback
    — see its docstring. Fails closed (returns False) on any error, so an
    ambiguous state never silently reactivates the old behavior.
    """
    sb = get_supabase()
    active_nodes = _execute_supabase(
        '_branch_is_single_node_unassigned.nodes',
        lambda: (
            sb.table('node_api_keys')
            .select('node_id')
            .eq('org_id', str(org_id))
    ...
    active_node_ids = {row.get('node_id') for row in (active_nodes.data or [])}
    if active_node_ids != {requesting_node_id}:
        return False

    any_assigned = _execute_supabase(
        '_branch_is_single_node_unassigned.assigned_cameras',
        lambda: (
            sb.table('branch_cameras')
            .select('id')
            .eq('organization_id', str(org_id))
            .eq('branch_id', str(branch_id))
            .not_.is_('assigned_node_id', 'null')
            .limit(1)
        ),
    )
    return not (any_assigned.data or [])
...
def get_node_config(node_api_key: str) -> dict:
    ...
    sb = get_supabase()
    # Cameras are scoped to THIS node's assigned_node_id, not to the branch
    # as a whole. A branch may now have more than one concurrently-active
    # node (see activate_node_with_install_token), each covering its own
    # subset of cameras — a camera with no assigned_node_id is returned to
    # nobody until an admin explicitly assigns it (see migration
    # 2026-09-18_camera_node_and_context_assignment.sql), rather than being
    # picked up by every node and double-processed.
    cameras_result = _execute_supabase(
        ...
        lambda: (
            ...
            .eq('enabled', True)
            .eq('assigned_node_id', str(node['node_id']))
            .order('channel')
        ),
    )
    ...
    # is NULL). Auto-heal that specific, narrow case: if this branch has
    # exactly one active node key (this one) and zero cameras have been
    # explicitly assigned to ANY node yet, fall back to "every enabled
    # camera on the branch belongs to this node" — the pre-migration
    # behavior. The instant a second node activates on the branch, or an
    # admin assigns any camera to any node, this fallback stops applying
    # for that branch and explicit assignment takes over.
    if not cameras:
        try:
            if _branch_is_single_node_unassigned(str(node['org_id']), str(node['branch_id']), str(node['node_id'])):
                fallback_result = _execute_supabase(
                    'get_node_config.cameras_fallback',
                    lambda: (
                        sb.table('branch_cameras')
                        .select('*')
                        .eq('organization_id', str(node['org_id']))
            ...
            logger.warning(
                ...
                'returning empty camera list rather than guessing',
