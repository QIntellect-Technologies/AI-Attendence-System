...
def _dashboard_cctv_status(cameras: list[dict], live_status: dict[str, dict] | None = None) -> list[dict]:
    """live_status is support_db_nodes.get_camera_live_status(org_id) — real
    per-camera online/offline derived from node heartbeats, not a static
    config column. Its entries are shaped {'status': 'Normal' | 'Offline' |
    'Not Synced', 'lastSeen': iso str | None} (see that function's
    docstring) — the same contract apply_camera_live_status() passes
    through to get_client_bootstrap() and /api/cctv/live-tracking, so this
    reads it identically rather than re-deriving 'online'/'last_frame_at'
    keys that don't exist on it. A camera missing from live_status (never
    reported, or only reported by a node that's since gone offline) renders
    'Not Synced' rather than defaulting to Normal, which is what previously
    let a disconnected/powered-off camera keep showing as fine on the
    dashboard.
    """
    live_status = live_status or {}
    result = []
    for idx, camera in enumerate(cameras):
        camera_id = _dashboard_text(camera.get('id') or camera.get('camera_id'), f'camera-{idx}')
        live = live_status.get(camera_id)

        if live is None:
            status = 'Not Synced'
            last_seen = 'Not Synced'
        else:
            status = live.get('status') or 'Not Synced'
            last_seen = _dashboard_text(live.get('lastSeen'), status)
...
def get_client_dashboard_overview(
    ...
    from support_db_nodes import get_camera_live_status
    camera_live_status = get_camera_live_status(org_key)
    cctv = _dashboard_cctv_status(list_client_cameras(org_key, backend_branch_id), camera_live_status)
    cctv_alerts = sum(1 for item in cctv if _dashboard_text(item.get('status')).lower() == 'alert')
