...
#                     WHERE id = ?
#                     """,
#                     (now, float(confidence), camera_id, check_in_metadata_json, notes, existing_dict["id"]),
#                 )
#                 conn.commit()
#                 return {
...
#                 WHERE id = ?
#                 """,
#                 (now, float(confidence), camera_id, check_in_metadata_json, hold_reason, notes, existing_dict["id"]),
#             )
#             conn.commit()
#             return {
#                 **existing_dict, "check_out_marked_at": now, "check_out_confidence": float(confidence),
...
#     """Operator action for hold_reason='early' rows only: the person left
#     before their checkout window opened, but the operator judges this a
...
#     Clears the tentative checkout (it was never real), keeps the
#     informative note already stored, and flags the day status='short_leave'
#     so downstream reporting/payroll can distinguish it from status='half_day'.
...
#             "UPDATE attendance_buffer SET check_out_marked_at = NULL, check_out_confidence = NULL, "
#             "check_out_camera_id = NULL, check_out_metadata = '{}', check_out_confirmed = 0, "
#             "check_out_hold_reason = NULL, status = 'short_leave', sync_status = 'pending', sync_error = NULL "
#             "WHERE id = ? AND check_out_hold_reason = 'early'",
#             (row["id"],),
#         )
#     return _resolve_held_checkouts(local_event_ids, build_update=build_update)
...
#     unlike leave_held_checkouts_open, this ACCEPTS the sighted
#     check_out_marked_at as the real, confirmed checkout time (the cloud's
#     overtime-hours computation needs a real check_out_timestamp to measure
#     against the shift's scheduled cutoff — see
#     support_db_attendance_exceptions._compute_overtime_hours), and flags
#     the day status='overtime' instead of leaving it untouched.

#     Mirrors confirm_held_checkouts (accept the sighted time, clear the
#     hold) but is restricted to hold_reason='late' only, same restriction
#     leave_held_checkouts_open already has — "overtime" only makes sense
#     for someone who stayed/was seen late, not someone who left early.
...
#         return (
#             "UPDATE attendance_buffer SET check_out_confirmed = 1, check_out_hold_reason = NULL, "
#             "status = 'overtime', sync_status = 'pending', sync_error = NULL "
#             "WHERE id = ? AND check_out_hold_reason = 'late'",
#             (row["id"],),
#         )
#     return _resolve_held_checkouts(local_event_ids, build_update=build_update)
