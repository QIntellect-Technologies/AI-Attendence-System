...
//   /** 'late' | null. Set only while a check-in sighting sits held for
//    * review because it arrived after the check-in window closed — the
//    * check-in-leg counterpart to check_out_hold_reason below. Drives
//    * whether the row shows the "confirm as check-in" / "mark half-day"
//    * actions. Null for a still-waiting early stray (no decision to make
//    * yet) and for confirmed rows. */
//   check_in_hold_reason?: "late" | null;
//   /** 'early' | 'late' | null. Set only for a held CHECKOUT sighting (as
//    * opposed to a held check-in, which uses check_in_hold_reason instead) —
//    * see local_db.py's record_attendance_local. Drives which of the three
//    * checkout resolution actions the row is eligible for. */
...
//   // The three held-checkout resolution actions share one request shape
//   // (a list of local_event_ids) and one response shape
//   // (ResolveHeldCheckoutsResponse) — only the endpoint path differs, so
//   // they're built off one small helper instead of repeating requestJson
//   // three times.
//   confirmHeldCheckouts: (localEventIds: string[]) =>
//     resolveHeldCheckouts(
//       "/api/held-attendance/confirm-checkout",
//       localEventIds,
//     ),
...
//   leaveHeldCheckoutsOpen: (localEventIds: string[]) =>
//     resolveHeldCheckouts("/api/held-attendance/leave-open", localEventIds),

//   // Check-in-leg counterpart to the two checkout actions above: a late
//   // check-in sighting held for review resolves to either "confirm as
//   // check-in" (accept the sighted time) or "mark half-day" (no real
//   // check-in that day). Same request/response shape, so reuses the same
//   // helper.
//   confirmHeldCheckIns: (localEventIds: string[]) =>
//     resolveHeldCheckouts("/api/held-attendance/confirm-checkin", localEventIds),
