...
//     "activate" | "refresh" | "cycle" | "sync" | "clear" | "restart" | null
//   >(null);
...
//       html:
//         "This ends the node process entirely and starts a fresh one — " +
//         "camera streams and this page will disconnect for a few seconds " +
...
//     setError(null);
//     try {
//       await localNodeApi.restart();
//     } catch {
//       // The process may die mid-response — that's expected, not a failure.
//     }
...
//         setError(
//           "Node restart is taking longer than expected. Please check the machine.",
//         );
//         setBusy(null);
//         return;
//       }
//       try {
//         await localNodeApi.status();
//         window.location.reload();
//       } catch {
//         window.setTimeout(() => void poll(), 1500);
//       }
//     };
//     window.setTimeout(() => void poll(), 2000); // give the old process a moment to actually die
//   };
...
//             type="button"
//             style={styles.secondaryButton}
//             onClick={() => void restartNode()}
//             disabled={busy !== null}
//           >
//             {busy === "restart" ? <Loader2 size={14} /> : <Power size={14} />}
//             Restart node
//           </button>
//         </div>
//       </header>
...
export default function App() {
  ...
  // treatment, deliberately; distinguishing "true" cold start from an
  // operator hitting F5 isn't worth a sessionStorage flag for a ~4s splash)
  // and re-shown whenever useSleepWakeDetector fires below.
  const [showSplash, setShowSplash] = useState(true);
  useSleepWakeDetector(() => setShowSplash(true));

  // Shown once, right after activation flips false -> true (edge-triggered,
  // not on every activated render — a refresh while already activated
  // shouldn't replay it).
  ...
  // Edge-triggered on the *real* false -> true transition only. `status`
  // is null until the first status fetch resolves, so `activated` reads
  // false on mount even for a node that's already activated — without
  // waiting for that first real response, this would misread "already
  // activated when the app opened" as a fresh activation and show the
  // welcome screen on every mount/reload instead of once, right after an
  // actual activate() call.
  useEffect(() => {
    if (!status) return;
    if (wasActivatedRef.current === null) {
...
function BrandHeader({ orgName }: { orgName: string }) {
  return (
    <div style={styles.brand}>
      ...
      <span style={styles.brandSub}>Powered by QIntellect Technologies</span>
    </div>
  );
}
