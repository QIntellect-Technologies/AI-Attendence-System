...
# # call (bypassing the dashboard's Math.max(0, ...) clamp on these same
# # inputs) could still post a negative lateComingPolicy amount/threshold,
# # leaveTypeQuotas entry, or allowanceTypes value straight into the engine.
# _PAYROLL_NON_NEGATIVE_NESTED_FIELDS = (
#     ('lateComingPolicy', 'thresholdOccurrences'),
#     ('lateComingPolicy', 'flatAmountPerOccurrence'),
# )


# def _validate_non_negative_number(value: object, label: str) -> None:
#     try:
#         num = float(value)
...
#     # Nested lateComingPolicy amounts -- same non-negative requirement as
#     # the top-level rate fields, and missing until now.
#     late_policy = policy.get('lateComingPolicy')
#     if isinstance(late_policy, dict):
#         for _, sub_field in _PAYROLL_NON_NEGATIVE_NESTED_FIELDS:
#             raw = late_policy.get(sub_field)
#             if raw in (None, ''):
#                 continue
#             _validate_non_negative_number(raw, f'lateComingPolicy.{sub_field}')

#     # leaveTypeQuotas: {leaveType: annual day count} -- the UI's min={0}
#     # step={1} clamp has no server-side mirror at all today.
#     leave_quotas = policy.get('leaveTypeQuotas')
#     if isinstance(leave_quotas, dict):
#         for leave_type, raw in leave_quotas.items():
#             if raw in (None, ''):
#                 continue
#             _validate_non_negative_number(raw, f'leaveTypeQuotas.{leave_type}')

#     # allowanceTypes: {key: {label, mode, value}} -- value is either a flat
#     # amount or a percent-of-basic; the UI clamps both to min={0} but a
#     # direct API call could still post a negative allowance.
#     allowance_types = policy.get('allowanceTypes')
#     if isinstance(allowance_types, dict):
#         allowance_labels: dict[str, str] = {}
#         for key, entry in allowance_types.items():
#             if not isinstance(entry, dict):
#                 continue
#             label = entry.get('label')
#             if label is not None:
#                 normalized_label = str(label).strip().casefold()
#                 previous_key = allowance_labels.get(normalized_label)
#                 if normalized_label and previous_key is not None:
#                     raise ValueError(
#                         f'allowanceTypes labels must be unique; '
#                         f'"{label}" duplicates allowanceTypes.{previous_key}.label'
#                     )
#                 if normalized_label:
#                     allowance_labels[normalized_label] = str(key)
#             raw = entry.get('value')
#             if raw in (None, ''):
#                 continue
#             _validate_non_negative_number(raw, f'allowanceTypes.{key}.value')
#             value = float(raw)
#             maximum = _PAYROLL_PERCENT_MAX if entry.get('mode') == 'percent' else _PAYROLL_VALUE_MAX
#             if value < _PAYROLL_VALUE_MIN:
#                 raise ValueError(f'allowanceTypes.{key}.value must be at least {int(_PAYROLL_VALUE_MIN)}')
#             if value > maximum:
#                 suffix = '%' if entry.get('mode') == 'percent' else ''
#                 raise ValueError(f'allowanceTypes.{key}.value cannot exceed {int(maximum)}{suffix}')
#             if label is not None and len(str(label)) > 40:
...
#         'rejectionNote': row.get('rejection_note'),
...
# def update_client_overtime_status(
#     overtime_id: str,
#     org_id: str,
#     status: str,
...
# ) -> dict:
#     """rejection_note is written to its own column, independent of
#     approved_by -- previously the reject-modal's note text was passed
#     into the approved_by argument by the caller and silently clobbered
#     who approved/rejected the request (see api_update_overtime). Only
#     touch the column when a note is actually supplied, so approving a
#     request (or rejecting one with no note) never blanks out a note
#     left by a prior decision on the same row."""
#     clean_status = str(status or 'approved').lower()
#     if clean_status not in {'approved', 'rejected', 'pending'}:
#         clean_status = 'approved'
#     now = datetime.now(timezone.utc).isoformat()
#     update_payload = {'status': clean_status, 'approved_by': approved_by, 'updated_at': now}
#     if rejection_note is not None:
#         # Same 500-char cap the frontend modal enforces client-side
#         # (RejectReasonModal.REASON_MAX_LENGTH) -- enforced here too so a
#         # direct API call can't bypass it.
#         update_payload['rejection_note'] = str(rejection_note).strip()[:500]
#     sb = get_supabase()
#     result = (
#         sb.table('overtime_requests')
...
_ATTENDANCE_REF_RE = re.compile(r'attendance_id=([0-9a-f-]{8,})', re.IGNORECASE)


def annotate_leave_payroll_treatment(org_id: str, mapped_rows: list[dict]) -> list[dict]:
    """Single source of truth for 'is this leave request paid, unpaid, or
    excluded from payroll' — every caller that lists leave requests
    (dashboard Leave Management, the HR assistant, tenant dashboard
    summary, payroll-period aggregation) must attach leave_compensation
    through this function rather than re-deriving it inline. Two call
    sites (support_db.list_client_leave_requests and
    support_db_attendance_dashboard.list_client_leave_requests) used to
    each carry their own copy of this ~50-line block; the copies had
    already drifted into the same bug (see below), which is exactly the
    failure mode duplicating policy-resolution logic invites.

    Mutates and returns `mapped_rows` in place. Each dict must already
    carry the _map_client_leave shape: at minimum 'reason', 'branch_id',
    and 'leave_type' (or 'type').

    Precedence per row:
      1. Attendance-adjustment leaves (reason embeds "(attendance_id=…)",
         or leave_type == 'attendance_adjustment') carrying an explicit
         check-out payroll decision: 'exclude' -> excluded, 'include' ->
         unpaid.
      2. Attendance-adjustment leaves with no decision yet -> unpaid
         (payroll includes them by default until explicitly excluded).
      3. Everything else -> the effective leaveTypeRules for THAT ROW'S
         OWN branch (org default, overridden per-branch — see
         get_leave_type_rules), not a single org-wide lookup applied to
         every row. Resolving per-row branch is what previous versions of
         this logic got wrong: they called get_payroll_policy(org_id)
         with no branch_id at all, so a branch-scoped leaveTypeRules
         override (set via the Payroll Rules modal when opened from a
         branch context) was never seen here — every leave request for
         that branch showed 'not_configured' regardless of what was
         actually saved.
    """
    if not mapped_rows:
        return mapped_rows
    ...
    # leaveTypeRules resolved lazily per branch and cached, so N rows
    # spanning only a handful of branches cost a handful of policy reads,
    # not N. '' is the sentinel for "row has no branch" -> org default.
    rules_cache: dict[str, dict[str, str]] = {}

    def _rules_for_branch(branch_id: str) -> dict[str, str]:
        cache_key = branch_id or ''
        cached = rules_cache.get(cache_key)
        if cached is not None:
            return cached
        raw_rules = get_leave_type_rules(org_key, branch_id=branch_id or None)
        normalized = {
            str(k).strip().lower(): str(v).strip().lower()
            for k, v in (raw_rules or {}).items()
            if str(k).strip() and str(v).strip().lower() in {'paid', 'unpaid'}
        }
        rules_cache[cache_key] = normalized
        return normalized

    # Attendance-adjustment linkage: rows whose `reason` embeds
    # "(attendance_id=<uuid>)" (see support_db.py's auto-recorded reason
    # text) are payroll-decision driven, not leaveTypeRules driven.
    linked_attendance_ids: set[str] = set()
    for row in mapped_rows:
        match = _ATTENDANCE_REF_RE.search(str(row.get('reason') or ''))
        if match:
            linked_attendance_ids.add(match.group(1))
    ...
    if linked_attendance_ids:
        try:
            result = _execute_supabase(
                'annotate_leave_payroll_treatment.attendance_payroll_decisions',
                lambda: (
                    get_supabase()
                    .table('attendance')
                    .select('id, check_out_payroll_decision')
            ...
            for row in (result.data or []):
                attendance_id = str(row.get('id') or '').strip()
                decision = str(row.get('check_out_payroll_decision') or '').strip().lower()
                if attendance_id and decision:
                    payroll_decision_by_attendance[attendance_id] = decision
        except Exception:
            payroll_decision_by_attendance = {}
    ...
    for row in mapped_rows:
        leave_type = str(row.get('leave_type') or row.get('type') or '').strip().lower()
        reason_text = str(row.get('reason') or '')
        match = _ATTENDANCE_REF_RE.search(reason_text)
        attendance_id = match.group(1) if match else ''
        payroll_decision = payroll_decision_by_attendance.get(attendance_id, '') if attendance_id else ''
        is_attendance_adjustment = attendance_id != '' or leave_type == 'attendance_adjustment'
        ...
        if payroll_decision == 'exclude':
            ...
            row_branch_id = str(row.get('branch_id') or row.get('backend_branch_id') or '').strip()
            leave_compensation = _rules_for_branch(row_branch_id).get(leave_type, 'not_configured')

        row['leave_compensation'] = leave_compensation
        row['leaveCompensation'] = leave_compensation
        row['leave_payroll_decision'] = payroll_decision or None
        row['leavePayrollDecision'] = payroll_decision or None
